...
def train_DP(device=2, is_cnn=True, is_save=False):
    print('DeepPacket_{} of class {} on device {} start to train...\n'.format('CNN' if is_cnn else 'SAE', len(CLASSES), device))
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch > 60 and max(all_f1[-5:]) < best_f1 and max(all_test_acc[-5:]) < best_acc:
            print('early stop with epoch: {}, \n'
                  'best test accuracy: {}, best f1-score: {}\n'.format(epoch, best_acc, best_f1))
            break


if __name__ == '__main__':
    train_DP(device=0, is_cnn=True, is_save=True)
    train_DP(device=0, is_cnn=False, is_save=True)
