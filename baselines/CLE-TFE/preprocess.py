...
def construct_dataset_from_bytes_ISCX(dir_path_dict, type):
    ...
    for category in dir_path_dict:
        ...
        data_list = []
        for file in tqdm.tqdm(file_list, desc=dir_path_dict[category][dir_path_dict[category].rfind('/')+1:]):
            if not (file.endswith('.npz') and '_' in file):
                continue
            file_path = dir_path + '/' + file
            # print('{} {} Process Starting'.format(show_time(), file_path))
            if opt.dataset == 'iscx-tor':
                data_list.extend(split_flow_Tor_nonoverlapping(
                    file_path, category, allow_empty=True, pad_trunc=True, config=config, type=type))
            else:
                data_list.extend(
                    split_flow_ISCX(file_path, category, allow_empty=True, pad_trunc=True, config=config, type=type))
            if len(data_list) >= config.MAX_SEG_PER_CLASS:
                break
...
def construct_graph_format_data(file_path, save_path, type, w_size=generalConfig.PMI_WINDOW_SIZE, pmi=1):
    ...
    label = file['label']
    print('total graph: {}'.format(len(data)))
...
if __name__ == '__main__':
    ...
    construct_graph_format_data(file_path=config.HEADER_TRAIN_DATA, save_path=config.HEADER_TRAIN_GRAPH_DATA,
                                type='header')
    construct_graph_format_data(file_path=config.HEADER_TEST_DATA, save_path=config.HEADER_TEST_GRAPH_DATA,
                                type='header')
